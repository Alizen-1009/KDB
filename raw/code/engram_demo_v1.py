...
class ShortConv(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        bsz, seq_len, groups, channels = x.shape

        assert groups == self.hc_mult, f"Input groups {groups} != hc_mult {self.hc_mult}"
        ...
        y = y_bct.transpose(1, 2).view(bsz, seq_len, groups, channels).contiguous()
